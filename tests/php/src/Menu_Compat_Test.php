 ...
 * cmd_compat_wpml_active and cmd_compat_polylang_active filters to
...
class Menu_Compat_Test extends ClassicMenuDuplicatorTestCase {
	...
	public function tear_down() {
		...
		remove_all_filters( 'cmd_compat_wpml_active' );
		remove_all_filters( 'cmd_compat_polylang_active' );
		remove_all_filters( 'cmd_item_meta_keys' );
		remove_all_filters( 'cmd_compat_excluded_meta_keys' );
		remove_all_actions( 'cmd_after_duplicate_menu_item' );
		remove_all_actions( 'cmd_after_import_item' );
	...
	public function test_no_hooks_registered_when_neither_plugin_active(): void {
		...
		$this->assertFalse( has_filter( 'cmd_item_meta_keys', array( $compat, 'filter_item_meta_keys' ) ) );
	}
	...
	public function test_hooks_registered_when_wpml_active(): void {
		add_filter( 'cmd_compat_wpml_active', '__return_true' );

		$compat = new Menu_Compat();
		$compat->register_hooks();

		$this->assertNotFalse( has_filter( 'cmd_item_meta_keys', array( $compat, 'filter_item_meta_keys' ) ) );
	}
	...
	public function test_hooks_registered_when_polylang_active(): void {
		add_filter( 'cmd_compat_polylang_active', '__return_true' );
	...
	// -----------------------------------------------------------------------
	// cmd_compat_excluded_meta_keys filter.
	// -----------------------------------------------------------------------
	...
	// -----------------------------------------------------------------------
	// cmd_before_duplicate_item action (added in Tier 3 to Menu_Duplicator).
	// -----------------------------------------------------------------------

	/**
	 * @covers Menu_Duplicator::duplicate_item
	 */
	public function test_cmd_before_duplicate_item_action_fires(): void {
		$menu_id = $this->create_menu_with_items( 'Before Item Test', 1 );
		$items   = wp_get_nav_menu_items( $menu_id );

		$fired = false;
		add_action( 'cmd_before_duplicate_item', static function () use ( &$fired ) {
			$fired = true;
		} );

		( new Menu_Duplicator() )->duplicate_item( $items[0]->ID, $menu_id );

		remove_all_actions( 'cmd_before_duplicate_item' );
	 ...
	 * @covers Menu_Duplicator::duplicate_item
	 */
	public function test_cmd_before_duplicate_item_receives_correct_args(): void {
