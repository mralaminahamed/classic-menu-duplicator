...
class Menu_Importer {
	...
	public function import(
		 ...
		 */
		$resolved_name = (string) apply_filters( 'cmd_import_menu_name', $resolved_name, $payload );
		 ...
		 * @param array<string,mixed> $payload Import payload.
		 */
		do_action( 'cmd_before_import_menu', $new_menu_id, $payload );
		 ...
		 */
		do_action( 'cmd_after_import_menu', $new_menu_id, $id_map, $payload );
	...
	private function insert_item( array $item, int $new_menu_id, string $find, string $replace ) {
		 ...
		 */
		do_action( 'cmd_after_import_item', $new_id, $item );
