...
class Menu_Compat {
	...
	public function register_hooks(): void {
		...
		add_filter( 'cmd_item_meta_keys', array( $this, 'filter_item_meta_keys' ) );

		// Strip any residual incompatible meta from a freshly duplicated item.
		add_action( 'cmd_after_duplicate_menu_item', array( $this, 'clean_item_meta' ), 10, 2 );

		// Strip incompatible meta from freshly imported items.
		add_action( 'cmd_after_import_item', array( $this, 'clean_item_meta' ), 10, 1 );

		if ( $this->wpml_active ) {
			// Ask WPML to register each new item in the translations table.
			add_action( 'cmd_after_duplicate_menu_item', array( $this, 'wpml_register_item' ), 20, 2 );
			add_action( 'cmd_after_import_item', array( $this, 'wpml_register_item' ), 20, 1 );

			// After a full menu is duplicated, register the new menu term itself.
			add_action( 'cmd_after_duplicate_menu', array( $this, 'wpml_register_menu' ), 10, 2 );
			add_action( 'cmd_after_import_menu', array( $this, 'wpml_register_menu' ), 10, 2 );
		}

		if ( $this->polylang_active ) {
			// Copy the language assignment of the source menu to the duplicate.
			add_action( 'cmd_after_duplicate_menu', array( $this, 'polylang_copy_menu_language' ), 10, 2 );
		}
	}
